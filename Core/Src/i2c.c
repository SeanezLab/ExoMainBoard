...
volatile uint32_t i2c_rx_err = 0;
volatile HAL_StatusTypeDef i2c_rx_status = HAL_OK;
...
{
    i2c_rx_done = 0;
    i2c_rx_status = HAL_OK;
    i2c_rx_err = 0;

    return HAL_I2C_Master_Receive_DMA(&hi2c2, FOLLOWER_ADDR_HAL, i2c_rx_buf, nbytes);
}
...
{
    if (hi2c->Instance != I2C2) return;
    i2c_rx_err = HAL_I2C_GetError(hi2c);
    i2c_rx_status = HAL_ERROR;
    i2c_rx_done = 1;
}
